...
// giving every button the same weight. Flat fill, no border, no gradient --
// reference shells (end-4, caelestia) use one solid color note for the
// primary action, not a lit-up effect.
Rectangle {
    id: root
    ...
    color: {
        if (root.variant === "primary") {
            return area.pressed ? Theme.mix(Theme.accent, Qt.rgba(0, 0, 0, 1), 0.12) : Theme.accent;
        }
